    ...
    >
      ...
      >
        ...
        <div class="w-full h-full justify-center flex items-center space-x-3">
          <Transition>
            <span
              v-show="props.status === 'PROCESSING'"
...
const props = defineProps(["status", "membershipId"]);
...
const statusChange = async (status) => {
  ...
  data.append("membershipId", props.membershipId);
  ...
  await $fetch("http://localhost:3333/management/updatemembershipstat", {
    ...
    .then((response, error) => {
      mainManagement.setMembershipChange();
    })
    .catch((error) => {
      console.log(error);
    });
