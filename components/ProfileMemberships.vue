...
const getOrders = async (userId: string) => {
  ...
  const { data } = await $fetch(
    "http://localhost:3333/membership/usermembership",
    {
      method: "POST",
      headers: {},
      body: body,
    ...
    .then(function (response) {
      ...
      response.memberships.forEach((membership) => {
        if (membership.status === "PROCESSING") {
          processingOrders.value.push(membership);
        }
        if (membership.status === "SHIPPING") {
          shippingOrders.value.push(membership);
        }
        if (membership.status === "DELIVERED") {
          deliveredOrders.value.push(membership);
        }
      });
      loading.value = false;
    })
